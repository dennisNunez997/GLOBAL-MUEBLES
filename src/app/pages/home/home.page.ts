...
export class HomePage implements OnInit {
  ...
  //promociones
  promosFiltrExist: any[];
  promosExistAngularList: AngularFireList<any>
  promosFiltr: any[];
  ...
  getPromociones(){
    this.promosExistAngularList = this.afs.list('promociones/')
    this.promosExistAngularList.snapshotChanges().subscribe(
      list => {
        this.promosFiltrExist = list.map(item => {
          return{
            $key: item.key,
            ...item.payload.val()
          }
        })
        this.promosFiltr = this.promosFiltrExist.filter(value => value.nombre_empresa)
        console.log("promosfiltr: "+this.promosFiltr)
      }
    )
  }

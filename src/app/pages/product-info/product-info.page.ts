...
export class ProductInfoPage implements OnInit {
  ...
  */
  ...
  //datos recibidos de modal-categoria
  @Input() nombre_prod;
  @Input() img_empresa_card
  @Input() nom_empresa_card

  //datos recibidos del modal-search producto
  @Input() nombre_producto_selected
  

  //observador
  observador: boolean = true;

  //si el producto ha sido seleccionado, mostrara la informacion del producto
  ...
  //productSelected from searchbar
  ProductSearchFiltrExist: any[];
  ProductSearchExistAngularList: AngularFireList<any>
  filtrProductSearchExist: any[];

  //categoria_card
  CategoryCardFiltrExist: any[];
  CategoryCardExistAngularList: AngularFireList<any>
  filtrCategoryCardExist: any[];
  ...
  ngOnInit() {
    ...
    this.ProductGetCCard()
    this.ProductGetSearch()
  }
  ...
  //funcion que visualiza el producto seleccionado del modal-search
  ProductGetSearch(){
    this.productSearch = false;
    this.mainProds = true;
    this.ProductSearchExistAngularList = this.afs.list('producto/')
    this.ProductSearchExistAngularList.snapshotChanges().subscribe(
      list => {
        this.ProductSearchFiltrExist = list.map(item => {
          return{
            $key: item.key,
            ...item.payload.val()
          }
        })
        this.filtrProductSearchExist = this.ProductSearchFiltrExist.filter(value => value.nombre_producto === this.nombre_producto_selected && value.empresa_proveedor === this.nom_empresa)
      }
    )
  } 


  //funcion que visualiza el producto seleccionado de modal-categoria
  ProductGetCCard(){
    this.productExist = false;
    this.mainProds = true;

    this.CategoryCardExistAngularList = this.afs.list('producto/')
    this.CategoryCardExistAngularList.snapshotChanges().subscribe(
      list => {
        this.CategoryCardFiltrExist = list.map(item => {
          return{
            $key: item.key,
            ...item.payload.val()
          }
        })        
        this.filtrCategoryCardExist = this.CategoryCardFiltrExist.filter(value => value.nombre_producto === this.nombre_prod && value.empresa_proveedor === this.nom_empresa)
  ...
  CategorySelected(){
    ...
    this.productSearch = true;
    console.log("productExist: "+this.productExist)
